const MODULES = [
  {
    ...
    label: "Collections",
    description:
      "Receive funds from payers through local bank transfers, card acceptance, and emerging stablecoin collection rails.",
  },
  {
    key: "disbursements",
    label: "Disbursements",
    description:
      "Deliver payouts through local rails, local currency wires, USD SWIFT, push-to-card, and future stablecoin disbursement options.",
  },
  {
    key: "accounts",
    label: "Accounts",
...
const PAYMENT_METHODS = [
  {
    ...
    description: "Traditional bank wires for high-value or time-flexible payment flows.",
  },
  {
    key: "nextDayAch",
    label: "Next day payments (ACH)",
    description: "Next-day ACH and comparable domestic clearing rails for efficient batch movement.",
  },
  {
    key: "fasterPayments",
    label: "Faster payment methods",
    description: "Real-time or same-day methods like RTP and faster ACH for speed-sensitive use cases.",
  },
  {
    key: "cards",
    label: "Cards (debit / credit)",
    description: "Debit and credit card funding or acceptance for checkout and invoice flows.",
  },
  {
    key: "stablecoin",
    label: "Stablecoin",
    description: "USDC and USDT rails for treasury movement, settlement, and wallet-linked experiences.",
  },
];
...
const STORAGE_PRODUCTS = [
  {
    ...
    description: "Named account details that help customers receive domestic or cross-border funds.",
  },
  {
    key: "virtualDebitCardAccounts",
    label: "Virtual debit card accounts",
    description: "Card-linked stored value experiences for controlled spending and online usage.",
  },
  {
    key: "stablecoinWallet",
    label: "Stablecoin wallet",
    description: "Wallet experiences for holding, sending, and receiving supported stablecoins.",
  },
  {
    key: "multiCurrencyWallet",
    label: "Multi-currency wallet",
    description: "Hold, convert, and move balances across major currencies from one wallet experience.",
  },
];
...
const ADDITIONAL_SERVICES = [
  {
    ...
    description: "Embedded payment acceptance inside checkout flows or invoicing journeys.",
  },
  {
    key: "invoicing",
    label: "Invoicing",
    description: "Invoice presentment, collection, and reconciliation experiences for payers.",
  },
  {
    key: "accountingIntegrations",
    label: "Accounting integrations",
    description: "ERP or accounting sync that keeps payment and ledger workflows in step.",
  },
  {
    key: "whiteLabelPayments",
    label: "Whitelabel payment services",
    description: "Payments embedded under your brand instead of a third-party UX.",
  },
  {
    key: "thirdPartyApi",
    label: "API use serviced through third party",
    description: "Payment APIs brokered through implementation partners or platform providers.",
  },
];
...
const STEP_CONTENT = {
  ...
  markets: {
    ...
    title: "Identify transaction types, markets, currencies, and methods.",
    description:
      "Select the countries, currencies, payment methods, and related services most relevant to your business.",
    footer:
      "Please focus on the markets most relevant to your near-term plans.",
  },
  collections: {
...
const state = {
  ...
  storageProducts: buildVolumeInterestMap(STORAGE_PRODUCTS),
  additionalServices: buildInterestMap(ADDITIONAL_SERVICES),
  collections: {
    senderTypes: [],
    payerCount: "",
    payerCountBasis: "actual",
    senderCountries: [],
...
function renderMarketsStep() {
  const selectedFlowPhrase = getSelectedFlowPhrase();
  const showStorageProducts = isModuleSelected("disbursements");
  const paymentMethodsIntro = isModuleSelected("collections") && !isModuleSelected("disbursements")
    ? "Please indicate the payment methods relevant to your incoming collection flows."
    : isModuleSelected("disbursements") && !isModuleSelected("collections")
      ? "Please indicate the payment methods relevant to your outgoing disbursement flows."
      : "Please indicate the payment methods relevant to your selected flows.";
    ...
    <div class="section-stack">
      <section class="section-card">
        ...
        ${renderChipSelector(
          ...
          TRANSACTION_TYPES.map((value) => ({ value, label: value }))
        )}
      </section>

      <section class="section-card">
        <h3>Markets and currencies</h3>
        <p class="section-card__intro">Please select the markets and currencies relevant to your current or planned ${escapeHtml(selectedFlowPhrase)}.</p>
      ...
      <section class="section-card">
        ...
        <p class="section-card__intro">${escapeHtml(paymentMethodsIntro)}</p>
      ...
      ${
        showStorageProducts
          ? `
            <section class="section-card">
              <h3>Money storage products</h3>
              <p class="section-card__intro">Please indicate the money storage products relevant to your disbursement or stored-balance experience.</p>
              ${renderInterestGrid(STORAGE_PRODUCTS, "storageProducts", true)}
            </section>
          `
          : ""
      }
      ...
      <section class="section-card">
        ...
        <div class="summary-grid">
          ...
          <div class="summary-card">
            ...
            <span>${state.operatingCountries.length} markets identified, ${state.currencies.length} currencies identified.</span>
          </div>
          <div class="summary-card">
    ...
    <div class="thank-you">
      <section class="thank-you__hero">
        ...
        <div>
          ...
          <span class="inline-stat"><strong>${state.operatingCountries.length || 0}</strong> markets identified</span>
          <span class="inline-stat"><strong>${countSelectedItems(state.paymentMethods)}</strong> payment methods indicated</span>
        </div>
              ...
              (doc) => `
                <article class="document-card">
                  ...
                  <div class="document-card__actions">
                    <button class="ghost-button" type="button" data-action="download-doc" data-doc-id="${doc.id}">
                      Download
                    </button>
                  </div>
                </article>
...
function validateRole() {
  ...
  if (!state.role.endCustomers.length) {
    errors.push("End customer");
  }
...
function validateMarkets() {
  ...
  if (!state.transactionTypes.length) {
    errors.push("At least one transaction type");
  }
  if (!state.operatingCountries.length) {
    errors.push("At least one operating country");
  }
  if (!state.currencies.length) {
    errors.push("At least one currency");
  }
...
function hasStablecoinInterest() {
  return (
    state.paymentMethods.stablecoin.current ||
    state.paymentMethods.stablecoin.interested ||
    (isModuleSelected("disbursements") &&
      (state.storageProducts.stablecoinWallet.current ||
        state.storageProducts.stablecoinWallet.interested))
  );
}
...
function buildSummary() {
  ...
  const storageSummary = isModuleSelected("disbursements")
    ? summarizeInterestGroup(STORAGE_PRODUCTS, state.storageProducts)
    : "";
  ...
  const lines = [
    ...
    `End customer: ${state.role.endCustomers.join(", ") || "N/A"}`,
    ...
    "",
    `Transaction types: ${state.transactionTypes.join(", ") || "N/A"}`,
  ...
  if (storageSummary) {
    lines.push(`Money storage products: ${storageSummary}`);
  }
